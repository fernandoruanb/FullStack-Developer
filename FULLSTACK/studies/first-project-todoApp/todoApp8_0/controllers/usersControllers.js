...
exports.recoverPassword = async (req, res) => {
	...
	let success = [];
	try {
		if (!req.session.permission || !req.session.email)
		...
		await usersModel.recoverPassword(password, req.session.email);
		req.session.permission = false;
		req.session.email = null;
		success.push("Successfully recover the password");
...
exports.getCodeValidate = async (req, res) => {
	try {
		...
		return res.render("verifyPasswordCode", {} );
	} catch (err) {
		req.session.email = null;
		req.session.permission = false;
...
exports.getEmailValidateCheck = async (req, res) => {
	let message = [];
	let success = [];
	...
	try {
		...
		await usersModel.checkEmailExistence(email);
		...
		const webPage = `
			<h2>Forgot Password TodoApp :)</h2>
			<p>Hello, that e-mail is to recover your lost password</p>
			<p>Inform that code to recover your password ${content}</p>
		`;
